...
const { startStockAutoRefresh } = require('./lib/stock-auto-refresh.js');
...
const client = new Client({
  intents: [
    ...
    GatewayIntentBits.MessageContent, // ✅ 추가: 임베드 description 등 메시지 내용을 읽으려면 필수
  ],
...
client.on('interactionCreate', async (interaction) => {
  ...
  // 2) 버튼 클릭, 모달 제출, 선택 메뉴 (구매로그 봇의 자동 버튼/패스 선택/닉네임 재입력 기능)
  if (interaction.isButton() || interaction.isModalSubmit() || interaction.isStringSelectMenu()) {
    if (
      interaction.customId.startsWith('retry_nick') ||    // 닉네임 다시 입력 버튼
      interaction.customId.startsWith('submit_nick') ||   // 닉네임 모달 제출
      interaction.customId.startsWith('autoform')         // ✅ 추가: 자동양식 버튼/모달/선택메뉴 전체 포함
                                                          //   (autoform, autoform_buyer, autoform_pass)
    ) {
...
client.on('messageCreate', async (message) => {
  ...
  // 자동 이모지 반응 (auto-react.js, 다른 기능과 완전히 독립)
  try {
    await handleAutoReact(message);
  } catch (err) {
    console.error('자동 이모지 반응 처리 중 오류:', err);
  }
});
