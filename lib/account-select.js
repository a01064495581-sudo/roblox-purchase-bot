...
function buildGuideEmbed(opts = {}) {
  ...
  if (accountIndex !== null && accountIndex !== undefined && ACCOUNTS[accountIndex]) {
    ...
    const priceLine = (robux !== null && robux !== undefined && !Number.isNaN(robux))
      ? `**${calculatePrice(robux)}**을 아래 계좌로 입금해 주세요.\n\n`
      : '아래 계좌로 입금해 주세요.\n\n';

    fields.push({
      name: '입금 안내',
      value: `${priceLine}${acc.bank} \`${acc.number}\` : **${acc.holder}**`,
      inline: false,
    });
  } else {
    fields.push({
  ...
  if (robux !== null && robux !== undefined && !Number.isNaN(robux)) {
    fields.push({ name: '\u200b', value: '\u200b', inline: false });
    fields.push({ name: '\u200b', value: '*(10,000원당 1,400로벅스 기준)*', inline: false });
  }
